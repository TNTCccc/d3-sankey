...
export interface SankeyLayout<Data, N extends SankeyExtraProperties, L extends SankeyExtraProperties> {
     ...
     */
    nodeSort(): (node0: SankeyNode<N, L>, node1: SankeyNode<N, L>) => number;
    /**
     * Set the node sort method the specified function and return this Sankey layout generator.
     *
     * @param nodeSort The specified function is used to set the vertical order 
     * of nodes in any given column, being passed two *node*s, and must return 
     * a number, which if is less than 0 places the first node above the second, 
     * and if greater than 0 places the second node above the first.
     */
